...
function normalizeTransfer(row, forcedType) {
  const directAmount = Number(firstDefined(row, ["amountUsd", "usdAmount", "amount", "value", "delta", "change", "quantity", "size", "qty", "netAmount", "assetAmount"]));
  const rawAmount = Number(firstDefined(row, ["rawAmount", "amountRaw", "amountWei"]));
  const decimals = Number(firstDefined(row, ["decimals", "tokenDecimals", "assetDecimals"]));
  ...
  const timestamp = normalizeTimestamp(firstDefined(row, ["timestamp", "createdAt", "updatedAt", "time", "executedAt", "blockTimestamp", "ts"]));
  const directionType = firstDefined(row, ["direction", "flow", "side"]);
  const boolDeposit = firstDefined(row, ["isDeposit", "deposit"]);
  const boolWithdrawal = firstDefined(row, ["isWithdrawal", "withdrawal"]);
  const directType = firstDefined(row, ["type", "eventType", "action", "txType", "kind"]);

  let type = forcedType;
  if (!type && (boolDeposit === true || boolDeposit === "true")) type = "Deposit";
  if (!type && (boolWithdrawal === true || boolWithdrawal === "true")) type = "Withdrawal";
  if (!type) type = normalizeTransferType(directType || directionType || (amount < 0 ? "withdrawal" : amount > 0 ? "deposit" : "transfer"));

  const tokenRaw = firstDefined(row, ["asset", "token", "symbol", "currency", "tokenSymbol"]);
  const token = typeof tokenRaw === "object"
    ? firstDefined(tokenRaw, ["symbol", "tokenSymbol", "name", "asset"]) || "USD"
    : tokenRaw || "USD";

  const txRaw = firstDefined(row, ["txHash", "transactionHash", "hash", "id", "txnHash", "tx"]);
  const txHash = typeof txRaw === "object"
    ? firstDefined(txRaw, ["hash", "txHash", "transactionHash"]) || "-"
    : txRaw || "-";
...
function extractTransferRows(payload, forcedType) {
  ...
  const out = [];
  for (const key of ["deposits", "withdrawals", "transfers", "history", "events", "entries", "records", "balanceChanges"]) {
  ...
  for (const containerKey of ["data", "result", "payload"]) {
    ...
    if (nested && typeof nested === "object") {
      out.push(...extractTransferRows(nested, forcedType));
    }
  }
  ...
  if (out.length) {
    ...
    out.forEach((row) => {
      const key = `${row.time}|${row.type}|${row.amount}|${row.txHash}`;
